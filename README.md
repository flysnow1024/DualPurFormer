...
## Release Status
This repository currently provides a method overview, experiment settings, summarized results and related word analysis. The complete model implementation and training and inference code will be released upon acceptance of the paper. This partial release does not yet support model training or inference.
...
### Evaluation Protocol
Experiments are conducted independently for each of the 12 participants on the 61-class DU-IN word decoding task. Following DU-IN, approximately 80% of the trials from each class are used for training, and the remaining trials are pooled and randomly divided equally into validation and test sets. Experiments use six random seeds: 42, 0, 1, 2, 3, and 4. Model selection is based on validation accuracy.
